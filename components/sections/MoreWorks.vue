<template>
    <section id="more-works" class="bg-primary text-light text-center py-5">
        <h4 class="mb-4 fs-1 fw-light">Wanna see more of our work ?</h4>
        <ul class=" list-inline">
            <li class="list-inline-item"><a href="" class="btn btn-lg btn-primary"><i class="bi bi-instagram"></i></a> </li>
            <li class="list-inline-item"><a href="" class="btn btn-lg btn-warning"><i class="bi bi-behance"></i></a> </li>
            <li class="list-inline-item"><a href="" class="btn btn-lg btn-danger"><i class="bi bi-youtube"></i></a> </li>
        </ul>
    </section>

</template>

<script>
export default {

}
</script>

<style>

</style>