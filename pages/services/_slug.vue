<template>
	<div>
			  ...
			  <div v-if="works.length > 0 || service.featured_image" class="col col-xl-4">
				<div v-if="service.featured_image" class="mb-5">
				  <img
					...
					loading="lazy"
					:src="`/assets/images/services/${service.featured_image}`"
					:alt="service.title"
				  />
				</div>
					  ...
					  <div class="ratio ratio-1x1">
						<div class="d-flex justify-content-center align-items-center">
						  <a :href="localePath(`/works/${work.slug}`)">
							<img
							  class="contained-image"
							  v-if="work.featured_image"
							  :src="`https://sixdegz.mo.cloudinary.net/promoe/assets/images/works/${work.featured_image}`"
							  :alt="work.title"
							/>
						  </a>
						</div>
	  ...
	  <!-- <PackageCallout
		class="rounded-3 p-5 bg-primary text-light text-center py-5"
		v-if="service.packages_link"
		:text="$t('packages_information_text')"
		:buttonText="$t('packages_information_button')"
		:link="localePath(service.packages_link)"
	  /> -->
  ...
  export default {
	...
	async asyncData({ $content, params, app }) {
	  ...
	  const works = await $content(app.i18n.locale, 'works')
		.where({ service: service.friendly_name })
		.fetch();
	  return {
		service,
		works,
